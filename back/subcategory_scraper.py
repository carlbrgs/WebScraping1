...
def scrape_subcategory(category_url):
    try:
...
if __name__ == "__main__":
    try:
        ...
        category = input("Entrez la catégorie (par exemple, 'web', 'marketing', etc.) : ").strip().lower().replace(" ", "-")
        if not category:
            raise ValueError("La catégorie ne peut pas être vide.")

        # Construire l'URL de la catégorie
        category_url = f"https://www.blogdumoderateur.com/dossier/{category}/"
        print(f"Scraping category: {category_url}")

        # Appeler la fonction pour scraper la catégorie
        scrape_subcategory(category_url)
